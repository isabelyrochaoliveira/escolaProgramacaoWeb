...
with st.expander("Filtrar Pets", expanded=True):
    ...
    with col_f1:
        filtro_status = st.selectbox(
            "Status", options=["Todos", "Disponível", "Adotado"], index=0
        )
    with col_f2:
...
if not pets_filtrados:
    ...
    for i in range(0, len(pets_filtrados), colunas_por_linha):
        ...
        for col, pet in zip(cols, grupo_pets):
            with col:
                caminho_foto = pet.get("foto")

                if caminho_foto:
                    caminho_normalizado = caminho_foto.replace("\\", "/")

                    if os.path.exists(caminho_normalizado):
                        st.image(caminho_normalizado, use_container_width=True)
                    else:
                        st.image(
                ...
                st.caption(
                    f"**Raça:** {pet.get('raca', '')} | **Porte:** {pet.get('porte', '')}"
                )
                st.caption(
                    f"**Sexo:** {pet.get('sexo', '')} | **Idade:** {pet.get('idade', 0)} ano(s)"
                )
                ...
                if pet.get("status_adocao") == "Disponível":
                    ...
                    if st.button(
                        ...
                        st.balloons()
                        st.success(
                            f"Parabéns pelo interesse em adotar **{pet.get('nome')}**! A equipe entrará em contato."
                        )
                else:
